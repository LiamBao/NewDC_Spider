...
public class Shell {
	 ...
	 */
	private static class IProxy extends ScriptExecuterDefaultImpl implements
			ContextAction, QuitAction {
	...
	public static int exec(String origArgs[]) {
		...
		return exec(origArgs, null);
	}

	/**
	 * Execute the given arguments, but don't System.exit at the end.
	 */
	public static int exec(String origArgs[], Global global) {
		if (global != null)
			Shell.global = global;
		errorReporter = new ToolErrorReporter(false, Shell.global.getErr());
		shellContextFactory.setErrorReporter(errorReporter);
		String[] args = processOptions(origArgs);
		if (processStdin)
			fileList.add(null);

		if (!Shell.global.isInitialized()) {
			Shell.global.init(shellContextFactory);
		}
	...
	public static String[] processOptions(String args[]) {
		...
		global.getOut().println(
				ToolErrorReporter.getMessage("msg.shell.usage", Shell.class
						.getName()));
		System.exit(1);
		return null;
	}
					...
					if (result != Context.getUndefinedValue()
						...
						try {
							...
							MessageDialogWrapper.showMessageDialog(debugGui,
									...
									"Run error for Selection",
									JOptionPane.ERROR_MESSAGE);
						}
					}
					NativeArray h = global.getHistory();
	...
	public static void processFile(Context cx, Scriptable scope, String filename) {
		if (securityImpl == null) {
			...
			securityImpl.doCallProcessFileSecure(cx, scope, filename);
		}
	}
	...
	public static Script loadScriptFromSource(Context cx, String scriptSource,
		...
		try {
			...
			MessageDialogWrapper.showMessageDialog(debugGui, errorMessage
					.toString(), "Run error for Selection",
					JOptionPane.ERROR_MESSAGE);
			exitCode = EXITCODE_RUNTIME_ERROR;
	...
	private static Script loadCompiledScript(Context cx, String path,
		...
		try {
			GeneratedClassLoader loader = SecurityController.createLoader(cx
					.getApplicationClassLoader(), securityDomain);
			...
			exitCode = EXITCODE_RUNTIME_ERROR;
			MessageDialogWrapper.showMessageDialog(debugGui, iaex.getMessage(),
					iaex.getMessage(), JOptionPane.ERROR_MESSAGE);
		} catch (InstantiationException inex) {
			exitCode = EXITCODE_RUNTIME_ERROR;
			MessageDialogWrapper.showMessageDialog(debugGui, inex.getMessage(),
					inex.getMessage(), JOptionPane.ERROR_MESSAGE);
		}
		return null;
	}

	private static SwingGui debugGui;

	/**
	 * @param debugGui
	 *            the debugGui to set
	 */
	public static void setDebugGui(SwingGui debugGui) {
		Shell.debugGui = debugGui;
	}
	...
	public static Object evaluateScript(Script script, Context cx,
		...
		try {
			...
			errorMessage.append(rex.getMessage());
			if (rex.lineSource() != null) {
				String lineSource = rex.lineSource().trim();
				if (lineSource.length() > 0 && !lineSource.equalsIgnoreCase("NULL")) {
	 ...
	 * @return
	 */
	public static Object evaluateScript(String scriptString, int startLine) {
		Context ctx = Context.enter();
		Script script = null;
		BufferedReader reader = new BufferedReader(new StringReader(StringUtil
				.readUTF8(scriptString)));
		try {
			script = ctx.compileReader(reader, "", startLine, null);
		...
		try {
			return com.cic.datacrawl.ui.shell.Shell.evaluateScript(script, ctx,
					com.cic.datacrawl.ui.shell.Shell.getGlobal());
		} finally {
			Context.exit();
		}
	}
	...
	private static Object readFileOrUrl(String path, boolean convertToString) {
		try {
			...
			MessageDialogWrapper.showMessageDialog(debugGui, ex.getMessage(),
					ToolErrorReporter.getMessage("msg.couldnt.read.source",
							path, ex.getMessage()), JOptionPane.ERROR_MESSAGE);
