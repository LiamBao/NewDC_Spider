...
import com.cic.datacrawl.management.dao.base.mysql.GroupBaseDAOImpl;
...
public class GroupDAOImpl extends GroupBaseDAOImpl implements GroupDAO {
	...
	public void cleanAgentCount(long[] groupIds) {
		if (groupIds == null || groupIds.length == 0)
			return;
		StringBuilder sql = new StringBuilder("update t_group set site_count=0 where id in (");
	...
	@Override
	public Group[] queryByName(String name, int start, int limit) {
	...
	public int calcSumSiteCount(long[] groupId) {
		if (groupId == null || groupId.length == 0)
			return 0;
		StringBuilder sql = new StringBuilder("select sum(site_count) from t_group where  id in (");
	...
	public int calcSumAgentCount(long[] groupId) {
		if (groupId == null || groupId.length == 0)
			return 0;
		StringBuilder sql = new StringBuilder("select sum(agent_count) from t_group where  id in (");
		for (int i = 0; i < groupId.length - 1; i++) {
			sql.append(groupId[i]).append(",");
		}
		sql.append(groupId[groupId.length - 1]).append(")");
