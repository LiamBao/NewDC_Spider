...
public class ConfigurationBaseDAOImpl extends JdbcDaoSupport implements ConfigurationBaseDAO{
	
	private static final Logger log = Logger.getLogger(ConfigurationBaseDAOImpl.class);
	...
	public Configuration getConfiguration(long id){
		String sql = "select id, name, value from t_configuration where id="+id;
		EntityProcessor processor = new EntityProcessor();
		log.info("SQL:"+ sql);
		getJdbcTemplate().query(sql, processor);
	...
	public Configuration[] getConfigurations(final long[] ids){
		...
		StringBuffer sql = new StringBuffer("select id, name, value from t_configuration where id in (");
	...
	public long addConfiguration(final Configuration configuration){
		...
		String sql = "insert into t_configuration (name, value) "
			+ "values(?,?)";
	...
	public int deleteConfigurations(long[] ids){
		...
		StringBuffer sql = new StringBuffer("delete from t_configuration where id in (");
		for (int i = 0; i < ids.length - 1; i++) {
			sql.append(ids[i]).append(",");
		}
